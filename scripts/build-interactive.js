...
/* Global border-box: the report-renderer reset covers its own markup, but
   dashboard-specific elements (.leg-row, .kpi-chip, .card paddings) were
   content-box by default — padding pushed them past their parents on mobile. */
.app-body, .app-body *, .app-header, .app-header * { box-sizing: border-box; }
...
/* Quote-tab sections: same horizontal rhythm as .sb-sec at every width */
...
@media (max-width: 1000px) {
  /* Mobile UX fix (2026-08): the desktop shell locks html/body overflow:hidden
     and gives .app-body a fixed flex height — correct for a side-by-side
     layout, but catastrophic stacked: the open drawer ran past the viewport
     bottom with NO way to scroll (results sat permanently below the fold).
     On narrow screens the page becomes a normal scrolling document; the app
     header sticks to the top so context stays visible while you scroll. */
  html, body { height: auto; overflow: visible; }
  .app-body { display: block; overflow: visible; }
  .results-col { display: block; min-height: 0; overflow: visible; }
  ...
  .sidebar {
    ...
    border-right: none;
    border-bottom: 1.5px solid var(--border);
    position: static; /* in normal flow — the DOCUMENT scrolls, not inner boxes */
  }
  .sidebar.open { max-height: none; overflow: visible; -webkit-overflow-scrolling: touch; }
  .drawer-btn {
    display: flex !important;
    position: sticky; top: 0; z-index: 30; /* stays reachable while scrolled */
    box-shadow: 0 2px 8px rgba(0,0,0,.08);
  }
  .results { padding: 16px 14px 48px; overflow-y: visible; }
  .two-col-grid { grid-template-columns: 1fr; }
  .hedge-cards { flex-direction: column; }
  .hedge-cards .h-card { min-width: unset; }
}

/* ── Mobile refinement at < 640px ─────────────────────────────────
   Phones: tables scroll horizontally inside their wrap (never break the
   viewport), KPI chips stack, header condenses, touch targets enlarge. */
@media (max-width: 640px) {
  /* Tables: every data-table scrolls inside its wrapper — kills the 173
     overflowing elements the mobile audit found. */
  .tbl-wrap, .data-table {
    overflow-x: auto;
    -webkit-overflow-scrolling: touch;
    scrollbar-width: thin;
  }
  .data-table { min-width: 620px; }   /* readable columns; wrap scrolls */
  .tbl-wrap > .data-table { margin-bottom: 8px; }

  /* KPI chips: full-width rows instead of a cramped row of three */
  .header-kpis { flex-direction: column; gap: 8px; width: 100%; }
  .kpi-chip { width: 100%; text-align: left; min-width: 0; display: flex; align-items: baseline; justify-content: space-between; gap: 12px; padding: 10px 14px; box-sizing: border-box; }
  .kpi-chip .kpi-value { font-size: var(--fs-kpi); }

  /* Header inner: stack everything vertically */
  .header-inner { grid-template-columns: 1fr !important; gap: 10px !important; padding: 14px 16px !important; }
  .header-logo { border-right: none !important; padding-right: 0 !important; }
  .trade-name { font-size: 18px !important; }

  /* Section/card padding tightened for narrow screens */
  .results { padding: 12px 12px 48px; }
  .card-body { padding: 14px 14px !important; }
  .card-footer { padding-left: 14px !important; padding-right: 14px !important; }
  section .card { overflow-x: hidden; }
  .wfsvg-wrap, .wf-reconcile, .wfsvg-legend, .wfsvg-collabels { padding-left: 14px !important; padding-right: 14px !important; }

  /* Close-out + comparison grids collapse to single column */
  #co-out + * , .card [style*="grid-template-columns:repeat(auto-fit"] { grid-template-columns: 1fr !important; }
  div[style*="grid-template-columns:repeat(auto-fit,minmax(180px,1fr))"] { grid-template-columns: 1fr !important; }
  div[style*="display:flex;gap:10px;align-items:center"] { flex-wrap: wrap; }

  /* Heatmap: scroll horizontally in its own box */
